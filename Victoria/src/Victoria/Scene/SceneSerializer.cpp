...
namespace Victoria
{
	...
	bool SceneSerializer::DeserializeText(const std::string& filepath)
	{
		...
		VC_CORE_TRACE("Deserializing scene '{0}'", sceneName);
		...
		if (entities)
		{
			for (auto entity : entities)
			{
				...
				VC_CORE_TRACE("Deserialized entity with ID = {0}, name = {1}", uuid, name);
	...
	void SceneSerializer::SerializeRendererSettings(YAML::Emitter& out)
	{
		...
		//out << YAML::Key << "ClearColor" << YAML::Value << m_EditorLayerContext->mClearColor;
